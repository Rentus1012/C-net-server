...
int main()
{
	...
	serv_addr.sin_addr.s_addr = inet_addr("127.0.0.1"); //로컬 서버의 ip 주소는 127.0.0.1(그 ip로 통신하게 하라.)
	//--------------------------------------------
	...
	while (1)
	{
		...
		if (sendsize <= 0)
			break;
		//------------------------------------------
		FILE* fp = fopen("log_client.txt", "at+");
		fprintf(fp, "%s\n", buf);//buf에 이미 닉이 들어가 있어서 buf만 하는거임.
		fclose(fp);//의미없이 열림을 방지할려고 flose 를 하는거임.
	}
